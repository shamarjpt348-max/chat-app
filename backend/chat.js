...
import { markConversationRead } from './readReceipts.js';

export const chatRouter = express.Router();
const uploadDir = path.resolve(process.env.UPLOAD_DIR || 'uploads');
fs.mkdirSync(uploadDir, { recursive: true });
const allowedMimeTypes = new Set(['image/jpeg', 'image/png', 'image/gif', 'image/webp', 'video/mp4', 'video/webm', 'video/quicktime', 'audio/mpeg', 'audio/wav', 'audio/ogg', 'audio/mp4', 'application/pdf', 'application/msword', 'application/vnd.openxmlformats-officedocument.wordprocessingml.document', 'application/vnd.ms-excel', 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet', 'text/plain']);
...
chatRouter.post('/conversations/:id/messages', upload.single('attachment'), (req, res) => {
  ...
  const messageType = attachment ? (attachment.type.startsWith('image/') ? 'image' : attachment.type.startsWith('video/') ? 'video' : attachment.type.startsWith('audio/') ? 'audio' : 'file') : 'text';
  const result = db.prepare('INSERT INTO messages (conversation_id, sender_id, body, reply_to_id, message_type, attachment_url, attachment_name, attachment_type, attachment_size) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)').run(req.params.id, req.user.id, parsed.data.body, null, messageType, attachment?.url || null, attachment?.name || null, attachment?.type || null, attachment?.size || null);
