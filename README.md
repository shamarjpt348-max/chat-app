...
The database and route boundaries are intentionally ready for the next modules: group membership tables, attachments with local file validation, message actions (reply/delete/forward), browser notifications, rate limiting, forgot-password token storage, and moderation/report workflows. These should be added as separate slices so each can be tested with two or more connected users.
